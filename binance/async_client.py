...
class AsyncClient(BaseClient):
    def __init__(
        ...
        if loop is not None:
            ...
            try:
                # Prefer an explicitly running loop to avoid DeprecationWarning from legacy helpers.
                self.loop = asyncio.get_running_loop()
            except RuntimeError:
                # No running loop — create a new loop as a safe fallback.
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse):
        ...
        if not str(response.status).startswith("2"):
            # Try to parse JSON body to provide structured error information,
            # fall back to plain text if parsing fails.
            try:
                body = await response.json()
            except Exception:
                body = await response.text()
            raise BinanceAPIException(response, response.status, body)
